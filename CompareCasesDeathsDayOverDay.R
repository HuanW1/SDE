...
#Compare deaths
#read in data from last day - update date if not yesterday
olddeaths <- read_csv(paste0("csv/", Sys.Date()-1, "/", Sys.Date()-1, "cases_wphi.csv")) %>% 
  filter(outcome=="Died")

#read in data from today to compare
newdeaths <- read_csv(paste0("csv/", Sys.Date(), "/", Sys.Date(), "cases_wphi.csv")) %>% 
  filter(outcome=="Died")
...
oldelr <- read_csv("elr_linelists/elr_linelist2020-12-15.csv")
...
newelr <- read_csv("elr_linelists/elr_linelist2020-12-15.csv")
...
newer <- newelr %>% 
  filter(mdy(lab_result_mod_date) >= mdy("12/15/2020"))
...
write_csv(newcasestoday, "csv/2020-10-13/2020-12-15_newcases.csv")
