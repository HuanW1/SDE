...
`r if(thursday){"__Community Transmission of COVID-19 __"}` 
  ...
  `r if(thursday){"Among "}` `r if(thursday){nrow(thursday_case)}` `r if(thursday){" new COVID-19 cases with specimen collection or onset date during "}` `r if(thursday){thursday_range}` `r if(thursday){", there were "}` `r if(thursday){nrow(thursday_case_nocong)}` `r if(thursday){" cases among people living in community settings, as shown in the map below. This corresponds to an average of "}``r if(thursday){round(nrow(thursday_case_nocong)/14/3572665*100000, 2)}`  `r if(thursday){" new COVID-19 cases per day per 100,000 population. Cases among people residing in nursing homes, assisted living facilities, and correctional facilities are excluded. Darker colors indicate towns with more cases."}`
...
thursday_town_table1 <- thursday_town_table[1:57,] %>% rename(town1 =Town, n1=Cases)
thursday_town_table2 <- thursday_town_table[58:114,] %>% rename(town2 =Town, n2=Cases)
thursday_town_table3 <- thursday_town_table[115:169,] %>% rename(town3 =Town, n3=Cases) %>% 
  mutate(
    n3 = as.character(n3),
