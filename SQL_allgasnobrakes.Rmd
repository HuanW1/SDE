...
thursday_town_table <- thursday_towncasecount_nocong %>%
  ...
  arrange(Town) %>% 
  replace_na(replace = list(Cases = "--", Rate = "--"))%>% 
  mutate(grp = rep(row_number(), length.out = n(), each = 57)) %>% 
  group_by(grp) %>% 
  mutate(group_row = row_number()) %>% 
  ungroup() %>% 
  pivot_wider(id_cols = c(group_row), 
              values_from = c(Town, Population, Cases, Rate), 
              names_from = grp) %>% 
  select(ends_with(paste0("_", as.character(1:3))))


thursday_town_table <- flextable::regulartable(thursday_town_table) 

names1 <- colnames(thursday_town_table)
names2 <- rep(c('Town','Population','Cases','Rate'), 3)

thursday_town_table <- set_header_df(x = thursday_town_table, 
                                   mapping = data.frame(keys = names1, 
                                                        values = names2, 
                                                        stringsAsFactors = FALSE), 
                                   key = "keys" )
...
thursday_town_table <- bold(thursday_town_table, part = "header")
thursday_town_table <- fontsize(thursday_town_table, size = 7, part = "all" )
...
thursday_town_table <- theme_booktabs(thursday_town_table)
...
gender_count %>% 
         group_by(gender) %>% 
         summarize(rate = round(sum(n)/tot*100000)) %>% 
         ungroup() %>% 
         unique() %>% 
ggplot()+
