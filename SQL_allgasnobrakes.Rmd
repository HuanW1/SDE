...
subdat14nc <- subdat %>% 
  inner_join(thursday_towncount_nocong, by = c("NAME" = "city"))
...
n14ncperc <- paste0("(",round(nrow(thursday_case_nocong)/nrow(thursday_case)*100), "%)")
com <- nrow(thursday_case) - nrow(thursday_case_nocong)
n14cperc <- paste0("(",round(com/nrow(thursday_case)*100), "%)")
```
...
test14 <- geocoded_tests
test14nc <- geocoded_tests %>%
  filter(!cong_test %in% "Yes")

subdat14tnc <- test14nc %>% 
  group_by(city) %>% 
  tally(name = "Tests") %>% 
  right_join(subdat, by = c("city" = "NAME"))
