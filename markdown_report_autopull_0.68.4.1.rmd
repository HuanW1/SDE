...
my_packages <-  c("ggspatial","data.table","sf","odbc","tm","viridis",
                  ...
                  "tigris", "acs", "tidycensus", "DT", "rgeos", "formatR", "knitr", "MMWRweek",
                   "plotly", 'rgeos', "scales", "english", 'flextable', 'DBI')
mypak(my_packages)
dir <- getwd()
...
#is it thursday? (sets output for report)
#change for thanksgiving
thursday <- wday(Sys.Date()) == 5
wedthurs <- wday(Sys.Date()) %in% c(4, 5)
...
```{r dependancies, include=FALSE}
dir.create("lab_data_here")
path1 <- paste0("csv/", Sys.Date())
dir.create(path1)
...
town_pop18 <- read_csv("population_data/pop_towns2018.csv") %>% 
  rename(city = Town,
         pop = `Est. Pop.`)
#genders <- read_csv("population_data/2018_dph_asrh/2018_asrh.csv")
cha_file <- list.files("CHA_data_here", pattern = ".csv", full.names = T)
...
#county populations
county_pop <- read_csv("population_data/county_pop.csv") %>% 
  mutate(County = paste0(County, " County"))

#adhoc read in
adhocfiles <- list.files("ad_hoc", pattern = ".csv")
if(!is_empty(adhocfiles)){
  dfadhoc <-  adhocfiles %>%
  map(~read_csv(file.path("ad_hoc", .), col_types = cols(
                                                               EventAgeYears = col_character(),
                                                               `Date admitted` = col_character(),
                                                               `Date discharged` = col_character(),
                                                               `Date of death` = col_character(),
                                                               `Hospital Name` = col_character(),
                                                               `Specimen Information (add a new lab result for each specimen): Specimen Collection Date` = col_character(),
                                                               `Died With COVID-19` = col_character(),
                                                               `Name of Facility` = col_character(),
                                                               `Testing Laboratory Information: Lab Facility`  = col_character(),
                                                               `Tests Performed on Sample/Specimen: Result`  = col_character(),
                                                               `Tests Performed on Sample/Specimen: Test or Method` = col_character(),
                                                               `If Yes, type of congregate setting` = col_character(),
                                                               `Did the patient reside in a congregate setting?` = col_character(),
                                                               `Was the case pregnant during illness?` = col_character(),
                                                               `Does patient work in a health care facility or setting?` = col_character(),
                                                               `What was their exposure to the congregate setting?` = col_character(),
                                                               `Has the patient attended a daycare facility` = col_character(), 
                                                               Daycare = col_character()))) %>%
  reduce(bind_rows) %>% 
  select_if(!str_detect(names(.), "X[0-9]")) %>% 
  mutate(
    `Chills:` = NA,
    `Cough:` = NA,
    `Difficulty breathing/Shortness of breath:` = NA,
    `Fatigue:` = NA,
    `Fever:` = NA,
    `Headache:` = NA,
    `Muscle aches/pains (myalgia):` = NA,
    `New olfactory or taste disorders` = NA,
    `Rigors / Shivers:` = NA,
    `Sore throat:` = NA,
    "New Case" = NA
  ) %>% 
  rename(
    `Event Age Years` = EventAgeYears,
    `Date Hospitalized` = `Date admitted`,
    `Date Discharged`=`Date discharged`,
    `Date of Death` = `Date of death`,
    `Died With COVID-19` = `Died with COVID-19`,
    `Specimen Date` = `Specimen Information (add a new lab result for each specimen): Specimen Collection Date`,
    `Result` = `Tests Performed on Sample/Specimen: Result`,
    `Test` = `Tests Performed on Sample/Specimen: Test or Method`,
    `Type of Congregate Setting` = `If Yes, type of congregate setting`,
    `Did Patient Reside or Spend Time in a Congregate Setting` = `Did the patient reside in a congregate setting?`,
    `Name of facility` = `Name of Facility`,   
    `Pregnant` = `Was the case pregnant during illness?`,
    `What Was Their Exposure to the Congregate Setting` = `What was their exposure to the congregate setting?`,
    `Case ID` = `Event ID`,
    `Case Report Form Entered`  = `Has case report form been entered?`,
    `Case Create Date` = `Create Date`,
    `Case Status` = Status,
    Phone = `Patient telephone # (home)`,
    Street = `Street address`,
    `Postal Code` = `Zip code`,
    `Current Official City` = `Official City`,
    Hispanic = `Is case Hispanic or Latino?`,
    `Healthcare Worker` = `Does patient work in a health care facility or setting?`,
    Hospitalized = `Was case hospitalized?`,
    `Hospital Name` = `Hospital name`,
    `Admitted to ICU` = `Was case admitted to the Intensive Care Unit (ICU)?`,
    `Symptom onset date:` = `Symptom onset date`,
    `Pneumonia (Clinical or radiological)` = `Pneumonia (Clinical or radiological)`,
    `Did the Patient Have ARDS` = `Did the patient have acute respiratory distress syndrome (ARDS)?`,
    `Investigation Create Date` = `Lab Results: Create Date`,
    `Investigation Modification Date` = `Lab Results: Modification Date`,
    MRNELR = `Specimen Information (add a new lab result for each specimen): MRN from ELR`,
    `Specimen Received Date`= `Specimen Information (add a new lab result for each specimen): Specimen Received Date`,
    `Specimen Number` = `Specimen Information (add a new lab result for each specimen): Specimen Number`,
    `Specimen Source`= `Specimen Information (add a new lab result for each specimen): Specimen Source`,
    `Date Tested` = `Tests Performed on Sample/Specimen: Date Tested`, 
    `Result Report Date` = `Tests Performed on Sample/Specimen: Date Result Reported to DPH`,
    `Ordering Lab Facility` = `Ordering Laboratory Information: Ordering Lab`,
    `Ordering Provider Facility` = `Ordering Provider or Authority Information: Facility`,
    `Ordering Provider Name` = `Ordering Provider or Authority Information: Name`,
    `New Lab Result Received via ELR` = `New Lab Result Received via ELR?`,
    Notes = `Notes: Staff Notes`,
    `Lab Facility Facility` = `Testing Laboratory Information: Lab Facility`,
    `Lab Facility Name` = `Testing Laboratory Information: ...Name`
  )

  
  dfadhoc_smushed <- dfadhoc %>% 
    filter(str_detect(Test, ";"))
  
  # smushed_results<- dfadhoc_smushed %>% 
  #   select(`Case ID`,  Result, `Specimen Date`) %>% 
  #   mutate(
  #   resultsplit = str_split(Result, pattern = ";")
  #     ) %>% 
  # unnest(resultsplit) %>% 
  #   mutate(resultsplit = str_trim(resultsplit))
  
   smushed_results_tests<- dfadhoc_smushed %>% 
    select(`Case ID`, `Specimen Number`, Result, Test, `Specimen Date`) %>% 
    mutate(
    resultsplit = str_split(Result, pattern = ";"),
    testsplit = str_split(Test, pattern = ";")
      ) %>% 
  unnest(cols = c(resultsplit, testsplit)) %>% 
    mutate(
      resultsplit = str_trim(resultsplit),
      testsplit = str_trim(testsplit)
           ) %>% 
     select(-c(Result, Test)) %>% 
     rename(Result = resultsplit,
            Test = testsplit
            )
  

  dfadhoc_smushed <- dfadhoc_smushed %>%
    select(-c(Result, Test)) %>% 
    left_join(smushed_results_tests, by = c('Case ID', 'Specimen Number','Specimen Date'))
  

  
 dfadhoc <- dfadhoc %>% 
   filter(!str_detect(Test, ";")) %>% 
   bind_rows(dfadhoc_smushed)
...
#data read in
files <- list.files("ctedss_data_here_2", pattern = ".csv")
df <-   files %>%
  map(~read_csv(file.path("ctedss_data_here_2", .), col_types = cols(`Chills:` = col_character(),
                                                               ...
                                                               `Lab Facility Facility` = col_character(),
...
if(!is_empty(adhocfiles)){
  ...
  dfadhoc <- dfadhoc %>% 
    select(dforder)
  df <- bind_rows(df, dfadhoc)
}


if(file.exists(paste0("yesterday/",Sys.Date()-1))){
  yesterday_file <- list.files(paste0("yesterday/",Sys.Date()-1), pattern = ".csv")
} else {
  dir.create("yesterday")
  dir.create(paste0("yesterday/",Sys.Date()-1))
...
#towns 
if(file.exists("shape_files/cb_2017_09_cousub_500k.shp")){
    filename<-list.files('shape_files', pattern=".shp", full.names=FALSE)
    filename<-gsub(".shp", "", filename)
    subdat <- st_read('shape_files', "cb_2017_09_cousub_500k")
    subdat<-st_transform(subdat,"+init=epsg:4326")
  } else {
    url<-"https://www2.census.gov/geo/tiger/GENZ2017/shp/cb_2017_09_cousub_500k.zip"
    destname<-"tiger_county.zip"
    download.file(url, destname)
    unzip(destname, exdir='shape_files', junkpaths=TRUE)
    filename<-list.files('shape_files', pattern=".shp", full.names=FALSE)
    filename<-gsub(".shp", "", filename)
    subdat <- st_read('shape_files', "cb_2017_09_cousub_500k")
    subdat<-st_transform(subdat,"+init=epsg:4326")
...
filename<-list.files('shape_files', pattern=".shp", full.names=FALSE)
  filename<-gsub(".shp", "", filename)
  subdat2 <- st_read('shape_files', "countyct_37800_0000_1990_s100_CENSUS_1_shp_wgs84")
...
# df$disease_status[df$eventid %in% c(101027326, 100906919)] <- "Probable"
# df$covid_death[df$eventid %in% c(102800742)]<- NA
# df$outcome[df$eventid %in% c(1002958107)]<- "Yes"

deathchangelist <- c(102739821, 103224555, 100961381, 103963349, 103922717, 101518798, 103458904, 102824455, 102849708, 103910753, 102680157) #add eventids separated by commas here, or something similiar to get a list
...
#12/21/2020 age change#
#df$age[df$eventid == 100915315] <- 74
#12/4/2020 age change#
#df$age[df$eventid %in% 101363224] <- 71
#12/7/2020 age change#
#df$age[df$eventid %in% 103686276] <- 84
...
# case$spec_date[!case$test %in% pcrtests] <- NA
# case$spec_col_date[!case$test %in% pcrtests] <- NA

thursday_case <- case %>% 
  filter(spec_date >= thursday_range_start & spec_date <= thursday_range_end )

#count(case, hisp_race2)
case2 <- case %>% 
  select(-phone) %>% 
  select(eventid,disease_status, age, gender, city, county, state, hisp_race, race, hisp,hospitalized, admit_date, discharge_date, icu, preg, symptoms, symp_onset_date, spec_col_date, fever, fatigue, sob, headache, cough, myalgia, new_olf_taste, rigors, pneumonia, ards, outcome, death_date, healthcare_worker, cong_setting, cong_exposure_type, cong_facility, cong_yn, create_date, event_date, mmwrweek)
write_csv(case2,paste0("csv/",Sys.Date(),"/", Sys.Date(), "cases.csv"))
rm(case2)

case3 <- case %>% 
select(eventid,fname, lname, dob, phone, disease_status, age, gender, Street, city, county, state, hisp_race, race, hisp,hospitalized, admit_date, discharge_date, icu, preg, symptoms, symp_onset_date, spec_col_date, fever, fatigue, sob, headache, cough, myalgia, new_olf_taste, rigors, pneumonia, ards, outcome, covid_death, vrn, ocme_reported, ocmeid, death_date, healthcare_worker, cong_setting, cong_exposure_type, cong_facility,cong_yn, create_date, event_date, test, result, lab_name, mmwrweek)
write_csv(case3,paste0("csv/",Sys.Date(),"/", Sys.Date(), "cases_wphi.csv"))
rm(case3)
case <- case %>% 
  select(-phone)
cdc_numbers <- case %>% 
...
write_csv(cdc_numbers,paste0("csv/",Sys.Date(),"/", Sys.Date(),"cdc_numbers.csv"))
```
...
outstate<-filter(df, `Ordering Provider Facility` %in% outstatecolleges)
elr <-   df %>% 
  rename(
    test_method = test,
    spec_num = `Specimen Number`,
    lab_result_create_date = `Investigation Create Date`,
    lab_result_mod_date = `Investigation Modification Date`,
    ordering_lab = `Ordering Lab Facility`,
    mrn_elr = `MRNELR`,
    lab_facility = `Lab Facility Facility`,
    name = lab_name,
    spec_rec_date = `Specimen Received Date`,
    date_tested = `Date Tested`,
    date_reported_dph = `Result Report Date`,
    #tests_performed_other = `Tests Performed on Sample/Specimen: Result Value or Other Type of Test`,
    #result_units = `Tests Performed on Sample/Specimen: Result Units`,
    #ref_Range = `Tests Performed on Sample/Specimen: Reference Range or standard`,
    auth_facility = `Ordering Provider Facility`,
    #auth_name = `Ordering Provider or Authority Information: Name`,
    ) %>%
  filter(!is.na(result)) %>% #filter out blank results
  filter(!auth_facility %in% outstatecolleges) %>% 
  select(-c(`Case Report Form Entered`, admit_date, ards, chills, City, cong_exposure_type, cong_facility, cong_setting, cough, covid_death, death_date, `Did Patient Reside or Spend Time in a Congregate Setting`, discharge_date, fatigue, fever, headache, healthcare_worker, hisp, hospital_name, hospitalized, icu, myalgia, new_olf_taste, outcome, `Postal Code`,  preg, race, satus, sob, sore_throat, rigors)) %>% 
  mutate(source = str_to_lower(source),
          source = ifelse(
      source %in% c("nasopharyngeal", "nasopharynx", "nasopharyngeal swab", "nasal", "nasal aspirate/swab" ),
      "np swab",
      ifelse(
        source == "nasopharyngel and oropharyngeal swab",
        "np and op swab",
        ifelse(
          source == "oropharyngeal/throat swab",
          "op swab",
          ifelse(
            source %in% c("bronchial wash", "bronchoalveolar lavage (bal)", "tracheal aspirate"),
            "bal",
            ifelse(
              is.na(source) | source=="other",
              "unspecified specimen",
 ...
 filter(state == "CT" | is.na(state)) %>%  #keep ct or blank state
   filter(!is.na(result)) %>% # keep non-NA results
   filter(test_method %in% pcrtests2| test_method %in% agtests) %>%  #keep only test methods we care about
  mutate( #new test var to distinct upon
    pcrag  = ifelse(test_method %in% pcrtests2,
                    "pcr",
                    "ag"
                    )
  ) %>%  
...
elr_linelist_elronly <-  elr_linelist %>% 
  filter(`New Lab Result Received via ELR` == "Yes") #%>%
...
total_pcr_tests <- nrow(elr %>%  filter(test_method %in% pcrtests2))  # change to PCR only and change name
total_ag_tests <- nrow(elr %>%  filter(test_method %in% agtests))

# 
# pcrtests<-elr %>%  filter(test_method %in% pcrtests2)
# agtestss<- elr %>%  filter(test_method %in% agtests)
# 
# 
# count(agtestss, spec_num) %>%  arrange(desc(n))
# count(agtestss, result) %>%  arrange(desc(n))
# count(agtestss, source) %>%  arrange(desc(n))
...
write_csv(elr_linelist, paste0("elr_linelists/elr_linelist", Sys.Date(),".csv"))
```
...
```{r overall_summary_table, echo=FALSE, message=FALSE, warning=FALSE}
...
tableforgary <- ovlsum
dir.create(paste0('yesterday/', Sys.Date()))
write_csv(ovlsum,paste0("yesterday/",Sys.Date(),"/", Sys.Date(), ".csv"))
...
chg_yst<- read_csv(paste0("yesterday/",Sys.Date()-1,"/",yesterday_file)) %>%
  select(`Total**`) %>%
...
*As of `r paste0(format(graphdate, "%m/%d/%y"), " ", params$desiredtime)`.*
```{r county_lab_cases_deaths_table, message=FALSE, warning=FALSE}
tbl_county <- tibble(County = as_factor(c("Fairfield County", "Hartford County", "Litchfield County", "Middlesex County", "New Haven County", "New London County", "Tolland County", "Windham County", "Pending address validation")))
...
#case rate grouping
# subdat14nc <- subdat14nc %>% 
#   mutate(avgrate = cut(CaseRate, breaks = c(-Inf,4.99,9.99,14.99,Inf), labels= avgratebreaks))
# subdat14nc$avgrate[is.na(subdat14nc$avgrate)] <-  "<5 cases per 100,000 or <5 reported cases"
...
```{r, eval = thursday}
#`r if(thursday){""}`
# caseman <- read_csv("C:\\Users\\asene\\Desktop\\thresholds\\cases_read\\2020-09-21cases_wphi.csv")

#CHANGED ON 10/8/2020 so that specimen collection date is used only for confirmed cases and grouped by epiweek
casebyweek <- case %>% 
  # mutate(date = ifelse(disease_status=="Confirmed" & !is.na(spec_col_date),
  #                           spec_col_date, event_date),
  #         date = as.Date(date, origin = "1970-01-01"),
  #        week=epiweek(date)) %>% 
  mutate(year = epiyear(event_date)) %>% 
...
ggplot(casebyweek)+
  ...
  annotate("rect", xmin =xaxis-1.5, xmax = xaxis+.5 , ymin = 0, ymax = ymaxweek, alpha = 0.5, fill = "#d0d1e6" )+
...
```{r, eval=thursday, dpi=300, fig.height=5, fig.width= 7}
#Case count map
ggplot(subdat14nc)+
  geom_sf(aes(fill = n2))+
...
#monthday <- "OCT_15"
# needed to be split up before...now maybe not so much. we'll see for now
if(wday(Sys.Date()) == 5){
  monthday <- format(Sys.Date()-1, "%b_%d")
...
# dir.create(paste0(dir,"/geodata"))
# geofile <- list.files("geodata", pattern = ".csv", full.names = T)
# alltestgeo <- read_csv(geofile)


#test
#alltestgeo <- read_csv("csv/2020-10-07/2020-10-02testgeo2.csv")
#alltestgeo <- read_csv("C:\\Users\\asene\\Downloads\\2020-08-21alltestgeo (1).csv")
#   alltestgeo <- read_csv("C:\\Users\\asene\\Downloads\\10_29_geo.csv")
# testgeo <- alltestgeo %>%
...
write_csv(newcasetable, paste0("csv/",Sys.Date(), "/", Sys.Date(), "newcases+tests.csv"))
```

```{r Forrest data set, eval=thursday}
#####################################################################################
#Forrest's data request
#count cases


cases_nc<- read_csv(paste0("csv/",Sys.Date(),"/", Sys.Date(), "cases_wphi.csv")) %>%
  mutate(cong_yn = if_else((str_detect(cong_exposure_type, "Reside")) &
                           (str_detect(cong_setting, "Jail / Prison") |   str_detect(cong_setting, "Long term care facility") | str_detect(cong_setting, "Assisted Living Facility")),
                           "Yes", "No", missing = "No")) %>% 
...
#output csv
write_csv(cases_tests, paste0("csv/",Sys.Date(),"/",Sys.Date(),"noncongtestconfirmcaseweek.csv"))
```
...
ggplot(dfs_age)+
  ...
  labs(title = paste0("Number of New COVID-19 Cases by Age Group \nwith Collection or Onset during ", thursday_range),
       #subtitle = paste0("As of ", format(graphdate, "%m/%d/%Y"), " at ",params$desiredtime),
       y = "Number of Cases",
       x="Age (years)")

# dfs_per_capita <- cases_7 %>%
#   ungroup() %>% 
#   group_by(age_group) %>% 
#   tally() %>%
#   filter(!is.na(age_group)) %>% 
...
#   theme(legend.position = "none")+
#   labs(
#     title = paste0("Rate of COVID-19 Cases by Age Group \nwith Collection or Onset during ", thursday_range),
#     #subtitle = paste0("As of ", format(graphdate, "%m/%d/%Y"), " at ",params$desiredtime),
...
graphdate <- Sys.Date()-1
ggplot(agegrp, aes(mmwrweekdate, rate_100k,col = age_group, group = age_group))+
  geom_line(size =1)+
  ylim(0, maxlim)+
  ...
  labs(
    title= "Average daily rate of COVID-19 cases\nby age group",
    subtitle = paste0("As of ", format(graphdate, "%m/%d/%Y"), " at 8:30PM"),
    x="",
...
```{r message=FALSE, warning=FALSE, fig.align= "center", eval=thursday, dpi=300, fig.height=5, fig.width= 7}

#incidence by county
countypop <- read_csv("population_data/county_pop.csv") %>%
  mutate(County = paste0(County, " County")) %>%
  rename(pop = Total)
...
`r if(thursday){"__Cumulative Number of COVID-19 Cases and COVID-19-Associated Death by Date__  "}`
...
```{r message=FALSE, warning=FALSE, fig.align= "center", eval = thursday, dpi = 300}
case_epi <- case %>% 
  filter(disease_status  == "Confirmed") %>% 
  group_by(spec_date) %>% 
  tally() %>% 
  filter(!is.na(spec_date)) %>% 
...
prob_epi <- case %>% 
 filter(disease_status  == "Probable") %>%
  mutate(date = ifelse(is.na(spec_col_date),
                            symp_onset_date,
                            ifelse(is.na(symp_onset_date),
                              event_date,
                              symp_onset_date
                            )
                       ),
...
```
```{r message=FALSE, warning=FALSE, fig.align= "center" , eval = thursday}
dfs_daily_dead <- case %>%
  ungroup() %>%
  filter(outcome == "Died") %>%
  select(death_date) %>%
...
```{r message=FALSE, warning=FALSE, dpi=300, fig.height=5, fig.width= 7}
ggplot(subdat2)+
...
#filter
#group tally
#geom_bar (bar takes data as is) vs geom_col (col like line)
...
deadbar <- case %>% 
  ...
  mutate(
    ...
    # cong_yn = if_else((str_detect(cong_exposure_type, "Reside")) &
    #                          (str_detect(cong_setting, "Jail / Prison") | str_detect(cong_setting, "Long term care facility") | str_detect(cong_setting, "[Aa]ssisted [Ll]iving [Ff]acility")),
    #                        "Yes", "No", missing = "No")
  ) %>% 
...
```{r daycare data, message=FALSE, warning=FALSE, fig.align="center", eval=thursday}
##########################
#### DAYCARE EDITS #######
##########################
...
daycare_attendee <- edit_daycare %>% 
  ...
  select(c(eventid, event_date, mmwrweek, week))%>%
  mutate(year = epiyear(event_date)) %>% 
  filter(week > 35 & year >=2020) %>% 
  group_by(year,week) %>% 
  tally()%>%
...
```{r message=FALSE, warning=FALSE, fig.align= "center", eval = not_thursday}
case_epi <- case %>% 
  filter(disease_status  == "Confirmed") %>% 
  group_by(spec_date) %>% 
  tally() %>% 
...
```{r message=FALSE, warning=FALSE, include=FALSE}
gender_race_eth <- read_csv("population_data/county_re_gender_age.csv")
```
...
```{r town_map, message=FALSE, warning=FALSE, dpi=500, fig.height=7, fig.width= 9 }
ggplot(subdat)+
  geom_sf(aes(fill = n))+
  geom_sf_text(aes(label = NAME), col = "black", size = 1.5)+
  scale_fill_brewer(palette = "Oranges")+
...
```{r message=FALSE, warning=FALSE, fig.align= "center", dpi=300, eval=thursday}
...
maxlimit2 <- (round(max(race_eth_comb$case_tot)/3000)*3000)+3000
if(maxlimit2<= max(race_eth_comb$case_tot)){
  maxlimit2 <- maxlimit2+3000
}
...
ggplot(
  ...
  scale_fill_brewer(palette = "Blues")+
  scale_y_continuous(limits = c(0,maxlimit2 ), breaks = seq(0,maxlimit2,3000))+
  theme_classic()+
  theme(legend.position = "none",
         axis.text.x=element_text(angle=45,hjust=1),
        )+
...
maxlimit3 <- (round(max(race_eth_comb$deaths)/500)*500)+500
if(maxlimit3<= max(race_eth_comb$deaths)){
  maxlimit3 <- maxlimit3+500
}
...
ggplot(
  ...
  scale_fill_brewer(palette = "Blues")+
   scale_y_continuous(limits = c(0,maxlimit3 ), breaks = seq(0,maxlimit3,500))+
  theme_classic()+
  theme(
    legend.position = "none",
     axis.text.x=element_text(angle=45,hjust=1)
...
```{r message=FALSE, warning=FALSE, fig.align= "center", dpi = 300}
stanpopas<-read_csv("dependancies/stanpopas.csv")

aamr_lookup <- read_csv("dependancies/2018_State-level_ASRH _lookup.csv") %>% 
...
```{r message=FALSE, warning=FALSE, fig.align= "center", dpi = 600, eval= FALSE}
...
path2 <- paste0("gary_csv/", Sys.Date())
dir.create(path2)
#state
...
write_csv(gary_state_file, paste0(path2,"/state_Result.csv"))

#town
town_codes <- read_csv("gary_csv/town_codes/Town_ID.csv") %>%
...
filter(!is.na(city) & !city %in% c("Not Available", "Pending validation")) %>% 
...
write_csv(gary_town_file, paste0(path2,"/town_Result.csv"))
...
write_csv(ConProbByDate, paste0(path2, "/ConProbByDate.csv"))
...
chg_yst2 <- read_csv(paste0("yesterday/",Sys.Date()-1,"/",yesterday_file)) %>%
  rename(yesterday = `Total**`) 
...
countylist <- c("New Haven County", "Hartford County",  "Middlesex County", "Fairfield County", "New London County", "Litchfield County", "Tolland County","Windham County")
...
cagstotalcases <- case %>%
  ...
  tally(name = "TotalCases") %>%
  ungroup() %>% 
  complete(age_group = age_labels , county = countylist, fill = list("TotalCases" = 0)) %>% 
  mutate(
    age_group = factor(age_group, levels = age_labels, labels = age_labels),
    TotalCases = ifelse(TotalCases <6 & TotalCases !=0, "<6", TotalCases)
         ) %>% 
  arrange(age_group) 
...
cagsconfcases <- case %>%
  filter(!is.na(age_group) & !is.na(county) & disease_status == "Confirmed") %>%
  group_by(county, age_group) %>%
  tally(name = "ConfCases") %>%
  ungroup() %>% 
  complete(age_group = age_labels, county = countylist, fill = list("ConfCases" = 0)) %>% 
  mutate(age_group = factor(age_group, levels = age_labels, labels = age_labels),
         ConfCases = ifelse(ConfCases <6 & ConfCases !=0, "<6", ConfCases)) %>% 
  arrange(age_group) 
...
cagsprobcases<- case %>%
  ...
  tally(name = "ProbCases") %>% 
  ungroup() %>%
  complete(age_group = age_labels, county = countylist, fill = list("ProbCases" = 0)) %>% 
  mutate(age_group = factor(age_group, levels = age_labels, labels = age_labels),
         ProbCases = ifelse(ProbCases <6 & ProbCases !=0, "<6", ProbCases)) %>% 
  arrange(age_group) 

 

cagstotaldeaths <- case %>%
  filter(!is.na(age_group) & !is.na(county) & outcome == "Died") %>%
  group_by(county, age_group) %>%
  tally(name = "TotalDeaths") %>%
  ungroup() %>%
  complete(age_group = age_labels, county = countylist, fill = list("TotalDeaths" = 0))%>% 
  mutate(age_group = factor(age_group, levels = age_labels, labels = age_labels),
         TotalDeaths = ifelse(TotalDeaths <6 & TotalDeaths !=0, "<6", TotalDeaths)) %>% 
  arrange(age_group) 

 

cagsconfdeaths <- case %>%
  filter(!is.na(age_group) & !is.na(county) & disease_status == "Confirmed" & outcome == "Died") %>%
  group_by(county,age_group) %>%
  tally(name = "ConfDeaths")%>%
  ungroup() %>%
  complete(age_group = age_labels, county = countylist, fill = list("ConfDeaths" = 0)) %>% 
  mutate(age_group = factor(age_group, levels = age_labels, labels = age_labels),
         ConfDeaths = ifelse(ConfDeaths <6 & ConfDeaths !=0, "<6", ConfDeaths)) %>% 
  arrange(age_group) 

 

cagsprobdeaths <- case %>%
  filter(!is.na(age_group)  & !is.na(county) & disease_status == "Probable" & outcome == "Died") %>%
  group_by(county, age_group) %>%
  tally(name = "ProbDeaths") %>%
  ungroup() %>% 
  complete(age_group = age_labels, county = countylist, fill = list(ProbDeaths = 0)) %>% 
  mutate(age_group = factor(age_group, levels = age_labels, labels = age_labels),
         ProbDeaths = ifelse(ProbDeaths <6 & ProbDeaths !=0, "<6", ProbDeaths)) %>% 
  arrange(age_group) 

 

county_age_pop <- gender_race_eth %>%  group_by(county, ageg) %>% summarise(n = sum(n)) %>% 
  mutate(county = paste0(county, " County"),
         ageg = if_else(ageg == ">= 80", ">=80", ageg)
         )
...
cagstotalrate <- case %>%
  ...
  group_by(county, age_group) %>%
  tally(name = "TotalCases") %>%
  left_join(county_age_pop, by = c("county" = "county", "age_group" = "ageg")) %>%
  mutate(TotalCaseRate = round((TotalCases/n)*100000))

 

CountyAgeGroupSummary <- tibble(
  AgeGroups = cagstotalcases$age_group,
  CountyTotalCases =cagstotalcases$TotalCases,
  CountyConfirmedCases = cagsconfcases$ConfCases,
  CountyProbableCases = cagsprobcases$ProbCases,
  CountyTotalDeaths = cagstotaldeaths$TotalDeaths,
  CountyConfirmedDeaths = cagsconfdeaths$ConfDeaths,
  CountyProbableDeaths = cagsprobdeaths$ProbDeaths,
  CountyTotalCaseRate = cagstotalrate$TotalCaseRate,
  DateUpdated = graphdate
) %>%
  mutate(AgeGroups = as.character(AgeGroups)) 
...
CountyAgeGroupSummary$AgeGroups[AgeGroupSummary$AgeGroups == ">=80"] <- "80 and older"

 


write_csv(CountyAgeGroupSummary, "CountyAgeGroupSummary.csv")
...
totaldeathsdate <- case %>% 
  ...
  complete(dateofDeath = seq.Date(as.Date("2020-01-01"), Sys.Date()-1, by = "day"), fill = list(deathCount = 0))
...
write_csv(countycountDate, paste0(path2, "/CountySummarybyDate.csv"))
write_csv(death_by_deathdate, paste0(path2, "/DodSummary.csv"))
write_csv(GenderSummary, paste0(path2, "/GenderSummary.csv"))
write_csv(StateSummary, paste0(path2, "/StateSummary.csv"))
write_csv(AgeGroupSummary, paste0(path2, "/AgeGroupSummary.csv"))
write_csv(CountySummary, paste0(path2, "/CountySummary.csv"))
write_csv(REStateSummary , paste0(path2, "/REStateSummary.csv"), na = "")
write_csv(agg_test_gary, paste0(path2, "/TestCounty.csv"))
...
gary_dailyincidence<-cases_14_nc %>% 
  ...
  replace_na(replace=list(casesweek1=0, casesweek2=0, n=0)) %>% 
  mutate(RateCategory=ifelse(avgrate =="<5 cases per 100,000 or <5 reported cases", "1. <5 cases per 100,000 or <5 reported cases",
                             ifelse(avgrate=="5-9 cases per 100,000", "2. 5-9 cases per 100,000", 
                                    ifelse(avgrate=="10-14 cases per 100,000", "3. 10-14 cases per 100,000",
                                           ifelse(avgrate=="15 or more cases per 100,000", "4. 15 or more cases per 100,000", "other")))),
         UpdateDate=Sys.Date(),
  ...
  rename(Town_No = TOWNNO) %>% 
  select(Town_No, NAME, pop, casesweek1, casesweek2, totalcases, CaseRate, RateCategory, TotalTests, PercentPositive, UpdateDate, ReportPeriodStartDate, ReportPeriodEndDate)
write_csv(gary_dailyincidence,paste0(path2, "/CTTown_Alert.csv"))
dir.create(paste0("yesterday/",Sys.Date(),"/avg_di"))
write_csv(gary_dailyincidence, paste0("yesterday/",Sys.Date(),"/avg_di/avgdailyincidence.csv"))
...
yestgdi <- read_csv(paste0("yesterday/", Sys.Date()-7, "/avg_di/avgdailyincidence.csv"))%>%
  mutate(
    RateCategory =str_trim(str_sub(RateCategory, start = 3)),
     RateCategory = factor( RateCategory, labels = lvls, levels = lvls)
         ) %>%
...
#   kable_classic(full_width = F, html_font = "Cambria")
 ...
 kable2 <-  gdi2 %>%
  ...
  #kable2
  dir.create("tables")
  dir.create(paste0("tables/", Sys.Date()))
  write_csv(kable2, paste0("tables/", Sys.Date(),"/TownAlertLevelsTable.csv"))
...
write_csv(Summary, paste0("tables/", Sys.Date(),"/SummaryAlertLevelsTable.csv"))
Summary
...
write_csv(modelingdata, paste0(path2, "/CT_daily_counts_totals.csv"))
...
write_csv(forrestdata, paste0("csv/", Sys.Date(), "/", Sys.Date(), "dailytest_communityonly.csv"))
...
sdedata <- testgeo2 %>%
  ...
  spread(result2, n) %>% 
  replace_na(replace=list(Positive=0, Negative=0, Indeterminate=0)) 
write_csv(sdedata, paste0("csv/", Sys.Date(), "/", Sys.Date(), "communitytest_county.csv"))

```


```{r county_data_request, message=FALSE, warning=FALSE, include=FALSE}
...
c7_logic <- case %>% 
  filter(spec_date %in% seq.Date(Sys.Date()-7, Sys.Date(), by = "day")) 
if(nrow(c7_logic ) >=1){
county7 <- case %>% 
  filter(spec_date %in% seq.Date(Sys.Date()-7, Sys.Date(), by = "day")) %>% 
  group_by(county) %>% 
...
write_csv(county7,paste0(path1,"/county7days.csv"))
```

```{r town_case_eids,  message=FALSE, warning=FALSE, include=FALSE}
thisweek <-  epiweek(Sys.Date())
townlhd <- read_csv("dependancies/towntolhd10_30.csv")
...
dir.create(paste0("town_case_eventids/", Sys.Date()))
mastercase <- cases %>% 
...
write_csv(mastercase, path = paste0("town_case_eventids/",Sys.Date(),"/","mastereventidlist.csv"))  
for(i in TOI){
  casesi <- cases %>%  
    filter(city == i) %>% 
    select(eventid)
dir.create(paste0("town_case_eventids/", Sys.Date(), "/",townlhd$`Health Department Name`[townlhd$Town == i]))  
write_csv(casesi, path = paste0("town_case_eventids/",Sys.Date(),"/",townlhd$`Health Department Name`[townlhd$Town == i] ,"/",i,"last2week.csv"))  
}
```
