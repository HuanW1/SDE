...
#### required packages should be in sys ####
require(rmarkdown)
require(tidyverse)
require(sf)
require(odbc)
...
# dir <- getwd() # always run this from .rproj don't set working directory
...
if(monthsame){
  ...
  thursday_range <- paste0(
    month(thursday_range_start, label = TRUE, abbr = FALSE), " ",
    format(thursday_range_start, "%d"), '-', month(thursday_range_end, label= TRUE, abbr = FALSE),
    " ",
    format(thursday_range_end, "%d")
  )
)
...
town_pop18 <- read_csv("L:/daily_reporting_figures_rdp/population_data/pop_towns2018.csv") %>% 
  rename(city = Town,
         pop = `Est. Pop.`)
#genders <- read_csv("L:/daily_reporting_figures_rdp/population_data/2018_dph_asrh/2018_asrh.csv")
...
#### data read in from ctedss_data_here_2 ####
files <- list.files("L:/daily_reporting_figures_rdp/ctedss_data_here_2", pattern = ".csv")
df <-
   files %>%
   map(~ read_csv(file.path("L:/daily_reporting_figures_rdp/ctedss_data_here_2", .),
                  col_types = cols( .default = col_character(),
                                    `Case ID` = col_double(),
                                    `Vital Records Death number` = col_double()),
                  progress = FALSE)) %>%
   reduce(bind_rows) %>%
   select_if(!str_detect(names(.), "X[0-9]"))
...
df <- df %>%
  ...
  rename(
    ...
    daycare_attendee = `Has the patient attended a daycare facility`,#AS 01/08/2020
    daycare_staff = Daycare#AS 01/08/2020
    )%>% 
...
write_csv(case2, paste0("L:/daily_reporting_figures_rdp/csv/", Sys.Date(), "/", Sys.Date(), "cases.csv"))
rm(case2)
...
#(case3,paste0("csv/",Sys.Date(),"/", Sys.Date(), "cases_wphi.csv"))
rm(case3)
case <- case %>% 
  select(-phone)
cdc_numbers <- case %>% 
...
write_csv(cdc_numbers, paste0("L:/daily_reporting_figures_rdp/csv/", Sys.Date(), "/",  Sys.Date(), "cdc_numbers.csv"))
...
write_csv(elr_linelist, paste0("L:/daily_reporting_figures_rdp/elr_linelists/elr_linelist", Sys.Date(), ".csv"))
...
```{r gary_text, message=FALSE, warning=FALSE, include=FALSE}
...
write_csv(ovlsum, paste0("L:/daily_reporting_figures_rdp/yesterday/", Sys.Date(), "/", Sys.Date(), ".csv"))
...
```{r not_sure2, eval=thursday, dpi=300, fig.height=5, fig.width= 7}
#Case count map
ggplot(subdat14nc)+
  geom_sf(aes(fill = n2))+
...
```{r not_sure3, eval=thursday, dpi=300, fig.height=5, fig.width= 7}
...
write_csv(newcasetable, paste0("L:/daily_reporting_figures_rdp/csv/", Sys.Date(), "/", Sys.Date(), "newcases+tests.csv"))
...
```{r not_sure5, eval=thursday, dpi=300, fig.height=5, fig.width= 7}
weeks <-  case %>%
  select(mmwrweek) %>%
  mutate(
    #year = 2020,
...
```{r not_sure6, message=FALSE, warning=FALSE, fig.align= "center", eval=thursday, dpi=300, fig.height=5, fig.width= 7}
...
```{r not_sure7, message=FALSE, warning=FALSE, fig.align= "center", eval = thursday, dpi = 300}
case_epi <- case %>% 
  filter(disease_status  == "Confirmed") %>% 
  group_by(spec_date) %>% 
  tally() %>% 
...
```{r not_sure8, message=FALSE, warning=FALSE, fig.align= "center" , eval = thursday}
dfs_daily_dead <- case %>%
  ungroup() %>%
  filter(outcome == "Died") %>%
  select(death_date) %>%
...
```{r not_sure10, message=FALSE, warning=FALSE, dpi=300, fig.height=5, fig.width= 7}
ggplot(subdat2)+
...
```{r not_sure11, message=FALSE, warning=FALSE, include=FALSE}
total_hosp <- cha %>%
...
```{r daycare_data, message=FALSE, warning=FALSE, fig.align="center", eval=thursday}
########### DAYCARE EDITS ###############
...
```{r not_sure12,  message=FALSE, warning=FALSE, fig.align= "center", eval=thursday, dpi=300}
...
`r if(thursday){"*Test results may be reported several days after specimen collection. Data are incomplete for most recent dates shaded in grey. Data for previous dates are routinely updated.*"}`
```{r not_sure13,  message=FALSE, warning=FALSE, fig.align= "center", eval=thursday, dpi=300}
elr_linelist %>% 
...
```{r not_sure14, message=FALSE, warning=FALSE, fig.align= "center", eval = not_thursday}
case_epi <- case %>% 
  filter(disease_status  == "Confirmed") %>% 
  group_by(spec_date) %>% 
  tally() %>% 
...
```{r not_sure15, message=FALSE, warning=FALSE, fig.align= "center" , eval = not_thursday}
dfs_daily_dead <- case %>%
  ungroup() %>%
  filter(outcome == "Died") %>%
  select(death_date) %>%
...
```{r not_sure16, message=FALSE, warning=FALSE, fig.align= "center", eval = thursday}
dfs_age <- case %>%
  group_by(age_group) %>% 
  tally() %>% 
  filter(!is.na(age_group))
...
ggplot(dfs_age)+
  ...
  labs(title = "Number of COVID-19 Cases\nby Age Group",
       ...
       x="Age(years)")
```
```{r not_sure17, message=FALSE, warning=FALSE, fig.align= "center", eval = thursday}
dfs_age_d <- case %>%
  ungroup() %>% 
...
```{r not_sure19,  message=FALSE, warning=FALSE, fig.align= "center" , eval = thursday}
gender_count <- case %>% 
  count(gender)
...
```{r not_sure20,  message=FALSE, warning=FALSE, fig.align= "center", eval = thursday}
ggplot(
  gender_count_dec %>% 
    filter(!is.na(gender) & !gender %in% c("Unknown", "Other"))
)+
...
write_csv(gary_state_file, paste0("L:/daily_reporting_figures_rdp/gary_csv/state_Result.csv"))
...
write_csv(gary_town_file, paste0("L:/daily_reporting_figures_rdp/gary_csv/town_Result.csv"))
...
write_csv(ConProbByDate, paste0("L:/daily_reporting_figures_rdp/gary_csv/ConProbByDate.csv"))
...
#agegroup summary by county  ###NEW WIP#####
# agstotalcases <- case %>% 
...
write_csv(countycountDate, paste0("L:/daily_reporting_figures_rdp/gary_csv/CountySummarybyDate.csv"))
write_csv(death_by_deathdate, paste0("L:/daily_reporting_figures_rdp/gary_csv/DodSummary.csv"))
write_csv(GenderSummary, paste0("L:/daily_reporting_figures_rdp/gary_csv/GenderSummary.csv"))
write_csv(StateSummary, paste0("L:/daily_reporting_figures_rdp/gary_csv/StateSummary.csv"))
write_csv(AgeGroupSummary, paste0("L:/daily_reporting_figures_rdp/gary_csv/AgeGroupSummary.csv"))
write_csv(CountySummary, paste0("L:/daily_reporting_figures_rdp/gary_csv/CountySummary.csv"))
write_csv(REStateSummary , paste0("L:/daily_reporting_figures_rdp/gary_csv/REStateSummary.csv"), na = "")
write_csv(agg_test_gary, paste0("L:/daily_reporting_figures_rdp/gary_csv/TestCounty.csv"))
...
write_csv(gary_dailyincidence,paste0("L:/daily_reporting_figures_rdp/gary_csv/CTTown_Alert.csv"))
dir.create(paste0("yesterday/",Sys.Date(),"/avg_di"))
write_csv(gary_dailyincidence, paste0("L:/daily_reporting_figures_rdp/gary_csv/yesterday/", Sys.Date(),"/avg_di/avgdailyincidence.csv"))
...
#  dir.create("tables")
  ...
  write_csv(kable2, paste0("L:/daily_reporting_figures_rdp/tables/", Sys.Date(),"/TownAlertLevelsTable.csv"))
...
write_csv(Summary, paste0("L:/daily_reporting_figures_rdp/tables/", Sys.Date(), "/SummaryAlertLevelsTable.csv"))
Summary
...
write_csv(modelingdata, paste0("L:/daily_reporting_figures_rdp/gary_csv/CT_daily_counts_totals.csv"))
...
#write_csv(forrestdata, paste0("L:/daily_reporting_figures_rdp/csv/", Sys.Date(), "/", Sys.Date(), "dailytest_communityonly.csv"))
...
sdedata <- testgeo2 %>%
  ...
  spread(result2, n) %>% 
  replace_na(replace=list(Positive=0, Negative=0, Indeterminate=0)) 
write_csv(sdedata, paste0("L:/daily_reporting_figures_rdp/csv/", Sys.Date(), "/", Sys.Date(), "communitytest_county.csv"))
...
write_csv(county7, paste0("L:/daily_reporting_figures_rdp/csv/county7days.csv"))
```
...
for(i in TOI){
  casesi <- cases %>%  
    filter(city == i) %>% 
    select(eventid)
dir.create(paste0("L:/daily_reporting_figures_rdp/town_case_eventids/", Sys.Date(), "/",townlhd$`Health Department Name`[townlhd$Town == i]))  
...
```{r casetow,  message=FALSE, warning=FALSE, include=FALSE, eval = FALSE }
#write_csv(case, "W:\\DosApps\\EPI\\EPISTAFF\\2019-nCoV\\tomsarchive\\cases_wphi.csv")
#write_csv(elr_linelist,"W:\\DosApps\\EPI\\EPISTAFF\\2019-nCoV\\tomsarchive\\elr_linelist.csv")
```

Time hack `r Sys.time()`

```{r casetosql,  message=FALSE, warning=FALSE, include=FALSE, eval=FALSE}
con2 <- DBI::dbConnect(odbc::odbc(), "epicenter")
#write and overwrite case to sql
DBI::dbWriteTable(con2, SQL('DPH_COVID_IMPORT.dbo.COVID_DAILY_CASE_OUTPUT'), case, overwrite = TRUE)

#write and overwrite elr_linelist to sql
DBI::dbWriteTable(con2, SQL('DPH_COVID_IMPORT.dbo.COVID_DAILY_TESTING_OUTPUT'), elr_linelist, overwrite = TRUE)

odbc::dbDisconnect(con2)
```


```{r casetosqlprac,  message=FALSE, warning=FALSE, include=FALSE, eval = FALSE}
# con2 <- DBI::dbConnect(odbc::odbc(), "COVID")
# 
# con3 <- RODBC::odbcConnect(dsn = "COVID")
# 
# odbc::dbWriteTable(conn = con2, name = "DPH_COVID_IMPORT.dbo.COVID_DAILY_CASE_OUTPUT",value = case, overwrite = TRUE )
# 
# odbc::dbDisconnect(con)
# # names(case)
# # identical(case$city, case$City)
# # 
# # cities <- case %>% 
# #   select(City,city)
# # cities2<- case %>% 
...
# # filter(city != City)
# 
# tables3 <- RODBC::sqlTables(con3)
# 
# sqlUpdate(channel = con3, tablename = tables3[12,], dat = case)
# 
# sqlSave(channel = con3, tablename = "dbo.COVID_DAILY_CASE_OUTPUT", dat = case, verbose = TRUE, append =FALSE, safer = FALSE)

# con4 <- DBI::dbConnect(odbc::odbc(), "COVID")
# con5 <- DBI::dbConnect(odbc::odbc(), "epicenter")
# # DBI::dbWriteTable(con4, SQL('dbo.COVID_DAILY_CASE_OUTPUT'), case[1,], overwrite = TRUE)
# 
# 
# DBI::dbWriteTable(con5, SQL('DPH_COVID_IMPORT.dbo.COVID_DAILY_CASE_OUTPUT'), case[1:3,], overwrite = TRUE)
#for network
#list.files("\\\\DPH-FS005\\FileShares")
```
