...
```{r SQL_cleanup, include = FALSE, message= FALSE}
...
df <- backend2 %>% 
  left_join(backend_tests2, by = c("event_id"))
# rm(backend)
# rm(backend_tests)
# rm(backend2)
# rm(backend_tests2)
 ...
 mutate(city = str_to_title(city)) %>% 
  ...
  mutate(
    race = ifelse(
      ...
      "Black",
      ifelse(
        race == "Asian"| race == "Native Hawaiian Other Pacific Islander",
...
case <- case %>% 
  ...
  mutate(
    date = ifelse(disease_status %in% c("Confirmed", "Probable") & !is.na(spec_col_date), spec_col_date, event_date),
         ...
         week = epiweek(date), 
         year = epiyear(date),
        spec_date = date,#just setting to date so it works for now
  ...
  mutate(cong_yn = if_else(
    (str_detect(cong_exposure_type, "Reside")) &
    (str_detect(cong_setting, "JAIL") | str_detect(cong_setting, "LTCF")),
                           "Yes", "No", missing = "No")
  ) %>% 
  group_by(bigID) %>%
...
write_csv(cdc_numbers, paste0("L:/daily_reporting_figures_rdp/csv/", Sys.Date(), "/",  Sys.Date(), "cdc_numbers.csv")) #who actually uses this?
