...
write_csv(case3, paste0("L:/daily_reporting_figures_rdp/csv/",Sys.Date(),"/", Sys.Date(), "cases_wphi.csv"))
...
casebyweek <- case %>% 
  ...
  group_by(year,week) %>% 
  tally() %>% 
  filter(!(year == 2020 & week <= 2) & !(year == 2021 & week >= 2)) %>%
  mutate(
...
odbc::dbDisconnect(con)
  ...
  monthday <- format(Sys.Date() - 2, "%b_%d")
...
totaldeathsdate <- case %>% 
  ...
  complete(dateofDeath = seq.Date(as.Date("2020-01-01"), Sys.Date(), by = "day"), fill = list(TotalDeathCount = 0))
confirmeddeathsdate <- case %>% 
  ...
  rename(dateofDeath = death_date) %>% 
  complete(dateofDeath = seq.Date(as.Date("2020-01-01"), Sys.Date(), by = "day"), fill = list(ConfDeathCount = 0))
probsdeathdate <- case %>% 
  filter(outcome == "Died" & disease_status == "Probable") %>% 
  filter(!is.na(death_date) & death_date >="2020-01-01") %>% 
  group_by(death_date) %>% 
  ...
  rename(dateofDeath = death_date) %>% 
  complete(dateofDeath = seq.Date(as.Date("2020-01-01"), Sys.Date(), by = "day"), fill = list(ProbDeathCount = 0))
...
# write_csv(gary_dailyincidence, paste0("L:/daily_reporting_figures_rdp/gary_csv/", Sys.Date(),"/avgdailyincidence.csv"))
write_csv(gary_dailyincidence, paste0("L:/daily_reporting_figures_rdp/yesterday/", Sys.Date(),"/avg_di/avgdailyincidence.csv"))
...
write_csv(county7, paste0("L:/daily_reporting_figures_rdp/csv/", Sys.Date(), "/", "county7days.csv"))
```
...
write_csv(mastercase, path = paste0("L:/daily_reporting_figures_rdp/town_case_eventids/", Sys.Date(), "/", "mastereventidlist.csv"))  
