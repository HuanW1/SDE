...
#Table to calculate % of pop in each category
popbycat <-town_alert %>% 
...
chanewadmit <- chalong %>%
  ...
  mutate(newadmit = ifelse(date==mdy("07-18-2020"), 18, 
                           ifelse(date==mdy("05-29-2020"), 35, 
                                  ifelse(date==mdy("03-22-2021"), 60, (value - lag(value))))),
...
ggplot(chanewadmit)+
  ...
  geom_line(aes(x=date, y=avgcount7), size=2)+
  theme_classic()+
  xlim(ymd("2020-04-13"), Sys.Date()-7)+
  labs(
...
## Maps for  number of tests & number of cases by town for last 3 weeks
...
ggplot(subdat_3wkc)+
  ...
  theme(
    ...
    legend.title = element_text(size = 11),
    legend.text = element_text(size=11)) +
  ggtitle("Average Daily Incidence Rate per 100,000 \nover last two weeks\n")
...
```{r vaccine_age_grp, echo=FALSE, fig.height=6, fig.width=8}
...
thursday <- wday(today()) == 5
wedthurs <- wday(today()) %in% c(5) # makes no sense this is the same as previous line
not_thursday <- wday(today()) != 5

thursday_range_start <- floor_date(today() - 12, unit = "week")
thursday_range_end <- thursday_range_start + 13
monthsame <- month(thursday_range_start) == month(thursday_range_end)
...
thisweek <- epiweek(today())
thisyear <- epiyear(today())
beginofcurmmwr <- MMWRweek2Date(MMWRyear = thisyear,
                                MMWRweek = thisweek,
                                MMWRday = 1)
...
weeks <-  case %>%
  ...
  # filter(mmwrweek >= epiweek(today())-9) %>%
...
nagegrp <- case %>%
  ...
  mutate(
    ...
    better_age_group = fct_relabel(age_group, ~ paste("Age", .))
  )

my_colors <- colorspace::sequential_hcl(n = nlevels(nagegrp$age_group),
                                        palette = "Plasma")

newggslopegraph(nagegrp,
                Times = mmwrweekdate,
