...
---
title: "Connecticut COVID-19 Weekly Data Review"
output:
...
my_packages <-  c("ggspatial","data.table","sf","odbc","tm","viridis",
                  "kableExtra","webshot", "geosphere", "pracma", "tidyverse", "tidytext", "ggplot2", "ggraph", "zoo", "roll",
                  ...
                  "RODBC", "plotly", 'rgeos', "scales", "english", 'flextable', 'slider', "transformr", "gganimate", "tibble", "dplyr")
mypak(my_packages)
dir <- getwd()
...
#6. create community only testing dataset 
...
elr_linelist <- read_csv(paste0("elr_linelists//","elr_linelist", Sys.Date(), ".csv")) %>% 
...
town_alert <- read_csv(paste0("tables/", Sys.Date(), "/TownAlertLevelsTable.csv"))

#8. CHA data
cha_file <- list.files("CHA_data_here", pattern = ".csv")
...
########################################### 2. SLIDE CREATION ######################
##Slide 1 - town alert map and % of population by alert level
...
#calculate % of pop in each category
popbycat <-town_alert %>% 
...
case_dat <- read_csv(paste0("gary_csv/", Sys.Date(), "/", "CTTown_Alert.csv"))
case_dat1 <- read_csv(paste0("gary_csv/", Sys.Date()-7, "/", "CTTown_Alert.csv"))
case_dat2 <- read_csv(paste0("gary_csv/", Sys.Date()-14, "/", "CTTown_Alert.csv"))
...
ggplot(case_dat1)+
  ...
  scale_fill_gradient(high = "#000099", low = "#ededff", limits=c(min(case_dat1$caserate),max(case_dat1$caserate)), guide = "colorbar") +
  theme_void()+
...
##Slide 2 - overall state snapshot
...
#b. graph of new hospitalizations by day over time
#set up CHA data 
...
#c. graph of COVID-19 hospital census by day over time 
chaadmit <- chalong %>%
...
#d. graph of deaths by day over time 
death_ct <- casenew %>%
  filter(outcome=="Died") %>% 
  group_by(death_date) %>% 
  tally() %>%
...
##Slide 3 - Recent cases in the past 12 weeks
...
##Slide 4 - Community test volume and positivity
...
#community testing for pcr only
...
```
##Slide 7 - new request for age group info from Albert. Community Tests ONLY. 
...
age_pop <- read_csv("population_data/2018_dph_asrh/2018_asrh.csv") %>% 
  #select(age_3, pop) %>% 
  #filter(!is.na(age_3)) %>%
  select(age_4, pop_4) %>% 
  filter(!is.na(age_4)) %>% 
...
#plot total tests over time by age group
...
#plot % positive tests over time by age group
...
##Slide 8 - map number of tests & number of cases by town for last 3 weeks
...
ggplot(subdat_3wkc)+
  ...
  theme_void()+
  labs(
    fill = "Average Daily Incidence Rate per 100,000 \nover last two weeks"
  )+
  theme(
    legend.position = "bottom",
    legend.direction = "vertical",
...
#Slide 9 - Turn Around Time
```{r slide 9, echo = FALSE}
##Slide 9 - turn around time = ALEX
elr <- testgeo2 %>% 
...
wrong <- elr %>% filter(tat > 30)%>% filter(spec_col_date >= end_date-20 & spec_col_date <= end_date-7)
...
#write_csv(wrong_foremail, "CRFs_entered_as_labs.csv")

#elr <- elr %>% filter(!eventid %in% wrong$eventid)
...
##Slide 16 - hospitalizations + Deaths
